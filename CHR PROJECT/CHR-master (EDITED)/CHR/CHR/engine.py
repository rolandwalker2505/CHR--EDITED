...
class Engine(object):
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if training:
            input_var = self.state['train_transform'](input_var)
        else:
            input_var = self.state['val_transform'](input_var)
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                ...
                loss_value = loss.cpu().numpy()[0] if isinstance(loss, torch.Tensor) else loss
                print('Epoch: [{0}]\t'
                      'Loss {loss:.4f}\t'
                      'mAP {map:.3f}'.format(self.state['epoch'], loss=loss_value, map=map))
            else:
                # print(self.state['ap_meter'].value())
                loss_value = loss.cpu().numpy()[0] if isinstance(loss, torch.Tensor) else loss
                print('Test: \t Loss {loss:.4f}\t  mAP {map:.3f}'.format(loss=loss_value, map=map))
